...
function style(chalk: Chalk, builder: Builder, args: unknown[]): string {
  const string = args.length === 1 ? '' + args[0] : args.join(' ')
  return chalk.level <= 0 || !string ? string : builder.open + string + builder.close
}

/**
 * Create!
 */
function createChalk(options: Options = {}) {
      ...
      (descriptors, [name, codes]) => {
        descriptors[name] = {
          get() {
            ...
            builder.open = (this as Builder).open ? (this as Builder).open.concat(open) : open
            builder.close = (this as Builder).close ? (this as Builder).close.concat(close) : close
        ...
        descriptors[model] = {
          get() {
            return (...args: any[]) => {
              ...
              builder.open = (this as Builder).open ? (this as Builder).open.concat(open) : open
              builder.close = (this as Builder).close
                ? (this as Builder).close.concat(close)
                : close
  ...
  Object.setPrototypeOf(chalk, Object.defineProperties({}, properties))
